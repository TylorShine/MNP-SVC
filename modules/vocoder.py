...
class CombSubMinimumNoisedPhase(torch.nn.Module):
    ...
    def forward(self, units_frames, f0_frames, volume_frames,
        ...
        pad_mode = 'constant'
        # stft, apply minimum-phase windowed sinc
        ...
        if self.use_noise_env:
            if not self.noise_to_harmonic_phase:
                ...
                noise_env_flat = (torch.log10(torch.clamp(ctrls['noise_envelope_magnitude'], min=0.0)*9. + 1.) + 1.).flatten(1)
            else:
                noise_env_flat = (torch.clamp(ctrls['noise_envelope_magnitude'], min=0.0) + 1.).flatten(1)
