...
class CombSubMinimumNoisedPhase(torch.nn.Module):
    def __init__(self, 
        ...
        # # functional stft from nnAudio
        # self.stft = feat.stft.STFT(n_fft=win_length,
        ...
        # self.stft_noise = feat.stft.STFT(n_fft=win_length,
        #                       win_length=win_length,
        #                       hop_length=block_size,
        #                       window='hann',
        #                       freq_bins=None,
        ...
        # self.istft = feat.stft.iSTFT(n_fft=win_length,
        #                         win_length=win_length,
        #                         hop_length=block_size,
        #                         window='hann',
        #                         freq_bins=None,
        ...
        # # multiplication of complex values
        # self.cmul_2d = lambda a, b: \
        #     torch.stack((a[:, :, 0]*b[:, :, 0] - a[:, :, 1]*b[:, :, 1], a[:, :, 0]*b[:, :, 1] + a[:, :, 1]*b[:, :, 0]), -1)
            
        # self.cmul_3d = lambda a, b: \
        #     torch.stack((a[:, :, :, 0]*b[:, :, :, 0] - a[:, :, :, 1]*b[:, :, :, 1], a[:, :, :, 0]*b[:, :, :, 1] + a[:, :, :, 1]*b[:, :, :, 0]), -1)
    ...
    def forward(self, units_frames, f0_frames, volume_frames,
        ...
        # src_filter = torch.exp(ctrls['harmonic_magnitude'] + 3.141592653589793j * ctrls['harmonic_phase'])
        # src_filter = torch.exp(ctrls['harmonic_magnitude'])
        # src_filter_i = src_filter * torch.sin(ctrls['harmonic_phase'])
        # src_filter = src_filter * torch.cos(ctrls['harmonic_phase'])
        # src_filter = torch.exp(ctrls['harmonic_magnitude'] + 3.141592653589793j * ctrls['harmonic_phase'])
        ...
        if self.use_noise:
            ...
            # noise_filter = torch.exp(ctrls['noise_magnitude'])
            # noise_filter_i = noise_filter * torch.sin(ctrls['noise_phase'])
            # noise_filter = noise_filter * torch.cos(ctrls['noise_phase'])
        ...
        if self.use_short_filter:
            # print(ctrls['short_harmonic_magnitude'].shape)
            ...
            # src_short_filter = torch.exp(ctrls['short_harmonic_magnitude'] + 0.j).reshape(x.shape[0], -1, 16)
            # src_short_filter = torch.exp(ctrls['short_harmonic_magnitude'] + 1.j * ctrls['short_harmonic_phase']).reshape(x.shape[0], -1, 16)
            ...
            # src_short_filter = torch.cat((src_short_filter, src_short_filter[:,-1:,:]), 1)
            # src_short_filter = torch.cat((src_short_filter, torch.zeros_like(src_short_filter[:,:,-1:])), 2).permute(0, 2, 1)
            # src_short_filter = torch.cat((src_short_filter, src_short_filter[:,:,-1:]), 2).permute(0, 2, 1)
            src_short_filter = torch.cat((src_short_filter, src_short_filter[:,-1:,:]), 1).permute(0, 2, 1)
            # src_short_filter = src_short_filter.permute(0, 2, 1)
            # print(src_short_filter.shape)
        ...
        if self.use_noise and self.use_noise_short_filter:
            ...
            # noise_short_filter = torch.exp(ctrls['short_noise_magnitude'] + 0.j).reshape(x.shape[0], -1, 16)
            # noise_short_filter = torch.exp(ctrls['short_noise_magnitude'] + 1.j * ctrls['short_noise_phase']).reshape(x.shape[0], -1, 8)
            ...
            # noise_short_filter = noise_short_filter.permute(0, 2, 1)
...
class CombSubMinimumNoisedPhase_export(torch.nn.Module):
    ...
    def forward(self, units_frames, f0_frames, volume_frames,
        ...
        # x_pad = torch.cat((x[:, 1:], torch.zeros_like(x[:, 0:1])), 1)
        # combtooth = torch.where(x_pad - x < 0., 1., 0.)
        # combtooth = combtooth.squeeze(-1)
        ...
        # torch._check_is_size(self.harmonic_env_size_downsamples.item())
        ...
        # noise_stft = torch.view_as_real(torch.stft(
        # noise_stft = torch.stft(
        #     noise_t,
        #     n_fft = self.win_length,
        #     win_length = self.win_length,
        ...
        # signal_stft = combtooth_stft + noise_stft * noise_filter
        
        # signal_stft = combtooth_stft + self.cmul_3d(noise_stft, noise_filter)
        ...
        # take the istft to resynthesize audio.
        # signal = torch.istft(
        #     # torch.view_as_complex(signal_stft),
        #     signal_stft,
        #     # combtooth_stft,
        #     # noise_stft,
        #     n_fft = self.win_length,
        ...
        #     combtooth_stft + noise_stft,
        #     combtooth_stft_i + noise_stft_i)
