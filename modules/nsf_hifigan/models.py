...
class ResBlock1c(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        super(ResBlock1c, self).__init__()
        self.h = h
        self.convs1 = nn.ModuleList([
            weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[0],
                               padding=get_padding(kernel_size, dilation[0]))),
...
class SourceModuleHnNSF(torch.nn.Module):
    ...
    def forward(self, x, upp):
        ...
        for i in range(x.shape[0]):
            sine_wavs = self.l_sin_gen(x[i], upp)
            sine_merge.append(self.l_tanh(self.l_linear(sine_wavs)))
        return torch.stack(sine_merge).squeeze(-1).transpose(2, 1)
...
class GeneratorDirectSource(torch.nn.Module):
    def __init__(self, h):
        super(GeneratorDirectSource, self).__init__()
        self.h = h
        self.num_kernels = len(h.resblock_kernel_sizes)
        self.num_upsamples = len(h.upsample_rates)
        self.noise_convs = nn.ModuleList()
