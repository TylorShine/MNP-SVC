...
class Unit2ControlGE2E(nn.Module):
    def __init__(
        ...
        self.dense_out = weight_norm(nn.Linear(n_hidden_channels, self.n_out))
        ...
        if spk_mix_dict is not None:
            if self.use_embed_conv:
                for i, (k, v) in enumerate(spk_mix_dict.items()):
                    x = x + v * self.spk_embed_conv(
                        spk_id[i].unsqueeze(-1).expand(x.shape[0], x.shape[2], x.shape[1]) +
                        f0_emb.transpose(2, 1) + 
                        self.phase_embed(phase / torch.pi).transpose(2, 1)).transpose(2, 1)
            else:
                for i, (k, v) in enumerate(spk_mix_dict.items()):
                    x = x + v * (
                        self.spk_embed(spk_id[i]) +
                        self.phase_embed(phase / torch.pi)).unsqueeze(1).expand(x.shape[0], x.shape[2], x.shape[1]).transpose(2, 1)
        else:
            if self.use_embed_conv:
                if self.use_spk_embed:
                    x = x + self.spk_embed_conv(
                        ...
                        f0_emb.transpose(2, 1) +
                        self.phase_embed(phase / torch.pi).transpose(2, 1))
                else:
                    x = x + self.spk_embed_conv(
                ...
                x = x + (
                    self.spk_embed(spk_id).unsqueeze(1).expand(x.shape[0], x.shape[1], self.n_hidden_channels) +
                    self.phase_embed(phase / torch.pi))
